...
class ParticlePanel(AppShell):
    ...
    def updateForceWidgets(self):
        ...
        if self.forceGroup is None:
            ...
            self.forceGroupNotebook.pack(fill = X)
            self.forcePageName = (
                f'{self.particleEffect.getName()}-{self.forceGroup.getName()}'
            )
    ...
    def addForceGroupNotebookPage(self, particleEffect, forceGroup):
        self.forcePageName = f'{particleEffect.getName()}-{forceGroup.getName()}'
    ...
    def createLinearForceWidgets(self, frame, pageName, forceName, force):
        ...
        def setVectorMasks(s=self, f=force, p=pageName, n=forceName):
            xMask = s.getVariable(p, f'{n} Mask X').get()
            yMask = s.getVariable(p, f'{n} Mask Y').get()
            zMask = s.getVariable(p, f'{n} Mask Z').get()
            f.setVectorMasks(xMask, yMask, zMask)

        self.createFloater(
            frame,
            pageName,
            f'{forceName} Amplitude',
        ...
        self.createCheckbutton(
            cbf,
            pageName,
            f'{forceName} Mass Dependent',
            'On: force depends on mass; ' + 'Off: force does not depend on mass',
            toggleMassDependent,
    ...
    def createLinearVectorForceWidget(self, forcePage, pageName,
        ...
        forceName = f'Vector Force-{repr(count)}'
    ...
    def createLinearFrictionForceWidget(self, forcePage, pageName,
        ...
        forceName = f'Friction Force-{repr(count)}'
        frame = self.createForceFrame(forcePage, forceName, force)
        self.createLinearForceWidgets(frame, pageName, forceName, force)
        self.createFloater(
            frame,
    ...
    def createLinearCylinderVortexForceWidget(self, forcePage, pageName,
                                              count, force):
        forceName = f'Vortex Force-{repr(count)}'
        def setCoef(coef, f = force):
            f.setCoef(coef)
    ...
    def createLinearDistanceForceWidget(self, forcePage, pageName,
        ...
        var = self.createOptionMenu(
            ...
            ('FT_ONE_OVER_R', 'FT_ONE_OVER_R_SQUARED', 'FT_ONE_OVER_R_CUBED'),
            command=setFalloffType,
        )
        self.getWidget(pageName, f'{forceName} Falloff').configure(label_width = 16)
